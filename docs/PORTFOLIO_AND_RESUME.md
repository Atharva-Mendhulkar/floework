...
> *"Kafka is an append-only distributed streaming log designed for event sourcing, stream processing, and multi-consumer pub/sub where messages need to be retained and replayed. For Floework, our requirement was discrete task execution: when a focus session completes, compute streak stability scores and generate an AI summary in strict chronological order per user. SQS FIFO provides ordered, deduplicated delivery semantics, while the worker uses idempotent processing to achieve effectively-once application behavior. Furthermore, SQS has no continuously running broker/cluster to pay for; costs are primarily request/data based. Provisioning an Amazon MSK Kafka cluster requires a minimum of 3 brokers running 24/7 ($150–$300/mo) and continuous partition rebalancing. SQS FIFO solves the actual problem with zero maintenance."*
...
> *"A single-AZ deployment has a catastrophic single point of failure: an Availability Zone impairment (power failure, fiber cut, or hardware degradation) causes a total application outage. By deploying across two Availability Zones in `us-east-1`, we place ECS tasks behind a multi-AZ Application Load Balancer, configure RDS PostgreSQL with a synchronous physical standby in the secondary AZ (with an automated failover design target < 120s), and run multi-AZ ElastiCache replication. If AZ-1 experiences an outage, Route 53 and ALB route traffic exclusively to AZ-2, and RDS automatically promotes the standby. We enforce multi-AZ in production, while maintaining single-AZ in staging to optimize cost."*
...
> *"IAM governance, auditability, and AWS ecosystem integration. If we used external AI APIs (OpenAI or Gemini public developer endpoints), application containers would rely on static API keys stored in secrets. With Amazon Bedrock, Claude 3 Haiku integrates natively with the AWS IAM security model: requests authenticate using standard AWS SigV4 signatures via the task execution role (`bedrock:InvokeModel`), appear in AWS CloudTrail audit logs, and can be accessed through AWS-managed regional endpoints (with VPC PrivateLink endpoints available if dedicated private connectivity without internet routing is required). Furthermore, our embedded Opossum circuit breaker ensures that any upstream AI timeout or throttling gracefully falls back to deterministic heuristic narratives."*

### Q10: What happens when the primary RDS database fails?
> *"In our Multi-AZ configuration, the primary RDS PostgreSQL instance synchronously replicates WAL blocks to a standby instance in a secondary AZ. If the primary instance crashes or experiences a hardware fault, AWS detects the heartbeat failure, promotes the standby instance to primary (architectural design target < 120s), and flips the database DNS CNAME record. In our application layer, the PostgreSQL client pool (`api/_lib/db.ts`) wraps all database transactions in an exponential backoff retry handler (`executeWithRetry`). When the failover occurs, transient connection reset errors (`ECONNRESET`, `57P01`) are caught, and queries automatically retry until the new primary is reachable, preventing container panics."*
...
> 2. **Scheduled Non-Production Scale-Down**: Use scheduled non-production shutdown/scale-down where supported, with explicit handling of RDS stop/start limitations (e.g., scaling ECS Fargate tasks to 0 on nights and weekends, while managing RDS single-AZ temporary stops for an estimated scenario saving ~65% in staging compute run-rate).
...
| :--- | :--- |
| **Disaster Recovery RPO** | *"We established an **architectural design target of RPO < 5 minutes**, supported by continuous RDS WAL archiving and S3 versioning, and validated point-in-time recovery via automated dry-run restoration scripts."* |
...
| **RDS Multi-AZ Failover** | *"Our architecture establishes an automated standby failover design target (< 120s); during standby promotion, our application connection pool implements exponential backoff retry wrappers (`executeWithRetry`) catching ECONNRESET/57P01 to ensure transparent reconnection once DNS updates without service disruption."* |
