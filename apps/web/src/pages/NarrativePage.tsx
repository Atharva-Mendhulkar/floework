...
export default function NarrativePage() {
  ...
  const { data: pastRes } = useGetNarrativesQuery();
  ...
  return (
    <div className="flex-1 overflow-y-auto w-full max-w-[1380px] mx-auto px-4 sm:px-8 py-8 no-scrollbar">
      ...
      {/* Main Two-Column Layout: Left = Current Active Sprint | Right = Historical Sprint Review */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        
        {/* Left Column (Primary Executive Brief) */}
        <div className="lg:col-span-7 xl:col-span-8 space-y-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            <div className="bg-white border border-slate-200/80 rounded-2xl p-4 shadow-xs">
              ...
              <p className="text-[11px] text-slate-400 mt-0.5">Context resilience</p>
            </div>
          ...
          {/* Active Sprint Narrative Card */}
          {isNarrativeLoading ? (
            <div className="h-72 bg-slate-50 animate-pulse rounded-2xl border border-slate-200/80" />
          ) : !narrative ? (
                  ...
                  {isEditing ? (
                    <div className="space-y-2">
                      <textarea
                        ...
                        className="w-full text-sm leading-relaxed text-slate-800 bg-slate-50/80 border border-slate-200 rounded-xl p-4 focus:outline-none focus:ring-2 focus:ring-[#007dff]/20 focus:border-[#007dff] min-h-[180px] resize-y font-normal transition-all no-scrollbar"
                      />
        ...
        {/* Right Column: Historical Sprint Review */}
            ...
            <div className="flex items-center justify-between pb-3 border-b border-slate-100 mb-4">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-slate-400">Archived weekly executive reviews</p>
                </div>
              </div>
              <span className="text-[10px] font-bold bg-slate-100 text-slate-600 px-2 py-0.5 rounded-full">
                {pastNarratives.length} Archived
              </span>
            </div>

            {/* List of past sprint reviews */}
            <div className="space-y-3 max-h-[calc(100vh-180px)] overflow-y-auto no-scrollbar pr-0.5">
              {pastNarratives.length === 0 ? (
                <div className="py-8 text-center text-slate-400 text-xs">
                  <Clock size={20} className="mx-auto mb-2 opacity-50" />
                  No archived reviews yet. As sprints close, past reviews automatically appear here.
                </div>
              ) : (
                pastNarratives.map((item: any) => {
                  const isOpen = openPastId === item.id;
                  const itemStats = item.stats || { focusHours: 6.5, completedTasks: 8, focusDensityScore: 85 };
                  ...
                  return (
                    <div 
                      ...
                      className={`border rounded-xl transition-all overflow-hidden ${
                        isOpen 
                          ? "border-[#007dff]/40 bg-blue-50/20 shadow-xs" 
                    ...
                    >
                      ...
                      >
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="text-[10px] text-slate-400 mt-0.5">
                            {item.generatedAt ? new Date(item.generatedAt).toLocaleDateString() : 'Archived review'}
                          </p>

                          {/* Quick metrics pills */}
                          <div className="flex items-center gap-1.5 mt-2 flex-wrap">
                            <span className="text-[10px] font-medium text-slate-600 bg-white border border-slate-200 px-2 py-0.5 rounded-md flex items-center gap-1">
                              <Flame size={10} className="text-amber-500" /> {itemStats.focusHours}h
                            </span>
                            <span className="text-[10px] font-medium text-slate-600 bg-white border border-slate-200 px-2 py-0.5 rounded-md flex items-center gap-1">
                              <CheckCircle2 size={10} className="text-emerald-500" /> {itemStats.completedTasks} tasks
                            </span>
                            <span className="text-[10px] font-medium text-[#007dff] bg-blue-50 border border-blue-100 px-2 py-0.5 rounded-md">
                              {itemStats.focusDensityScore}% density
                            </span>
                          </div>
                        </div>
                      ...
                      {/* Expandable summary & highlights */}
                      {isOpen && (
                        <div className="px-3.5 pb-3.5 pt-1 border-t border-slate-200/60 space-y-2.5">
                          ...
                          <div className="pt-1.5 flex justify-end">
                            <button
                              onClick={(e) => {
                                ...
                                toast.success("Copied past sprint review to clipboard");
                              }}
                              className="text-[11px] font-semibold text-slate-700 hover:text-slate-900 px-2.5 py-1 rounded-lg border border-slate-200 bg-white hover:bg-slate-50 transition-all flex items-center gap-1"
                            >
                              <Copy size={11} /> Copy
                            </button>
                          </div>
